...
async function getJSON<T>(url: string, parentSignal?: AbortSignal): Promise<T> {
  const { signal, cleanup } = createTimeoutSignal(parentSignal);
  try {
...
export const libraryV2 = {
  async getSnapshotPage(cursor = '', limit = 500, signal?: AbortSignal): Promise<LibrarySnapshotPage> {
    const params = new URLSearchParams({ limit: String(limit) });
    if (cursor) params.set('cursor', cursor);
    return getJSON(`${API_V2_BASE}/library/snapshot?${params.toString()}`, signal);
  },
  ...
  async getChanges(since: number, signal?: AbortSignal): Promise<{ revision: number; changes: LibraryChange[]; songs: Song[] }> {
    ...
    return { revision: cursorRevision, changes, songs: Array.from(songsById.values()) };
  },

  async search(query: string, limit = 50, signal?: AbortSignal): Promise<ClientLibrarySearchResult> {
    const params = new URLSearchParams({ q: query, limit: String(limit) });
    const result = await getJSON<LibrarySearchResult>(`${API_V2_BASE}/search?${params.toString()}`, signal);
...
export function searchResultPlaylistToPlaylist(result: LibrarySearchPlaylist): Playlist {
  return { id: result.id, name: result.name, songIds: [], createdAt: 0 };
}
