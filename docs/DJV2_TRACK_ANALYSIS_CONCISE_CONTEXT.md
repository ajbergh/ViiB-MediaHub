...
The refusal-calibration experiment froze `tempo.minOnsetCrestFactor=12` after r5 tuning: thresholds 10 and 12 both reached 101/122 strict (82.79%), two half/double errors, and two unknowns, improving the production-threshold-15 replay by three strict matches and four fewer unknowns. The more conservative tied value, 12, then received one r5 held-out run: **44/54 strict (81.48%)**, two half/double errors, and one unknown. Tempo-confidence buckets are monotonic (79.17%/82.35%/91.67%), but strict BPM accuracy still fails qualification. This threshold remains a recorded benchmark candidate, not a production change and must not be retuned on held-out results. Local immutable artifacts are `phase0-go-tuning-r5-crest12-20260918-v1.json`, `phase0-tuning-r5-crest12-comparison-20260918-v1.json`, `phase0-go-tuning-r5-crest10-20260918-v1.json`, `phase0-tuning-r5-crest10-comparison-20260918-v1.json`, `phase0-go-heldout-r5-crest12-20260918-v1.json`, and `phase0-heldout-r5-crest12-comparison-20260918-v1.json`.
...
Local audit artifacts: `phase0-spotify-manifest-r4.json`, `phase0-spotify-import-r4.json`, `phase0-corpus-content-hashes-r3.json`, `phase0-identity-subset-audit-r5.json`, `phase0-corpus-coverage-r5.json`, and `phase0-corpus-readiness-r5-20260918-v1.json` under `sample_media/Test Corpus/`. `analysisbench -audit-manifest <manifest> -audit-out <new.json>` is a metadata-only, non-overwriting intake audit: it never opens local audio and reports the fixed 200-track/67-held-out qualification gaps, declared recording identity, every required coverage tag, and held-out `stable-electronic` evidence. The r5 audit reports the correct 24-track and 13-held-out gaps (rather than one third of its incomplete current size), 176 distinct declared groups, zero cross-split groups, no held-out stable-electronic evidence, and all 15 required coverage tags missing. The gate now requires recording-group identity and blocks repeated/cross-split groups rather than treating copies as independent evidence. Legacy manifests still load, but missing identity is an explicit readiness deficit.
