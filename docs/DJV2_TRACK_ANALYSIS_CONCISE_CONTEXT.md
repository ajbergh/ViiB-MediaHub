...
The new development-only `scripts/reference_bpm_benchmark.py` normalizes external BPM references into the existing benchmark schema without becoming a ViiB dependency. Its first whole-track `librosa` 0.11.0 control on the r5 tuning split used FFmpeg PCM at 22050 Hz, a 512-sample hop, `startBPM=120`, and `tightness=100`: 13/122 strict BPM (10.66%), one half/double error, no source failures, 157.99 seconds. The same-manifest production Go replay is 98/122 strict (80.33%), two half/double errors, and six unknown BPM results. Librosa's default tracker is therefore a reproducible control rather than a promotion candidate. See `docs/REFERENCE_BPM_BENCHMARK.md`; raw local artifacts remain under `sample_media/Test Corpus/`. Essentia and Mixxx/Queen Mary remain separate AGPL/GPL development-only references to evaluate in their own environments.
