...
These are directional development-browser samples, not controlled benchmarks, native WebView measurements, or proof of glitch-free audio. Both playback positions advanced without state resets during the stress samples. Some frame delays remain. Application-shell work remains a profiling candidate; native platform frame measurements still require their respective WebView runtimes.
...
- The Canvas fallback now caches static overview peaks until its track data, dimensions, or device-pixel ratio changes. Its normal RGB and single-color scrolling waveforms batch all bars into one fill per deck per frame. Dynamic playheads, cues, and beat markers still draw each frame. `DJ_AUDIT_DISABLE_WEBGL=1 node scripts/dj-overlay-audit.mjs` exercises this fallback at every audited size.

## Interaction and catalog validation

`node scripts/dj-overlay-audit.mjs` uses an isolated browser context, intercepted 12,000-song catalog/playlist responses, and generated WAV audio. It checks all five geometry pairs, fewer than 80 mounted rows (9–13 observed), search focus, hidden state, Escape from search, column-menu priority, shortcut-modal priority, text-entry safety, retained search, sorting, playlist filtering, column visibility, load A/B, drag-to-deck, playback continuity, Browse, fullscreen, and the width floor. Synthetic records are never written to the real backend catalog. Set `DJ_AUDIT_DISABLE_WEBGL=1` to repeat this suite with the Canvas fallback.

The live 3,014-song catalog was also browsed, searched, sorted and loaded into both decks. Deck A alone, Deck B alone, and both-deck playback were exercised with crossfader/EQ/tempo changes and animated waveforms/meters. Local audio requests loaded successfully. A follow-up now distinguishes formats intentionally generated by Web Audio: they return HTTP 422 and take the existing client fallback instead of being recorded as server HTTP 500 failures. Extension matching is case-insensitive, so `.MP3` follows the server MP3 path. True server generation failures still return HTTP 500. The synthetic audit also exercises Deck B playback/load behavior; no native audible evaluation is claimed.
...
- Follow-up native smoke validation built the updated Windows executable, launched it with an isolated data directory and non-default API port, and confirmed a responsive Wails process, HTTP 200 from `/api/health`, active WebView2 child processes, and the embedded UI's initial API requests. Maximized/fullscreen interaction, native CPU/GPU frame profiling, and audible-glitch testing remain outstanding.
...
- `DJ_AUDIT_DISABLE_WEBGL=1 node scripts/dj-overlay-audit.mjs`: passes the same suite through the Canvas fallback.
- `cd backend; go test ./...`: passes.
- `cd backend; CGO_ENABLED=1 go test -race ./...` with MinGW on PATH: passes.
- Follow-up Windows scanner remediation: USN record filename ranges are checked before decoding, and the computed-pointer UTF-16 array conversion was removed. `go vet ./internal/scanner` passes on Windows; malformed or truncated records are skipped safely.
