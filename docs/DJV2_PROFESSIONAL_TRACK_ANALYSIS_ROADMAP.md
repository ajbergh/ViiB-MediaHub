...
For key, the explicit Temperley profile option improved tuning Camelot compatibility from 64.57% to 66.93%, and independently improved held-out compatibility from 56.14% to 64.91% (37/57); exact held-out key was 40.35%. A six-harmonic chroma-summation candidate reached 68.50% on tuning but only 59.65% held out, so it was rejected rather than retained as a scanner option. The 85% held-out Camelot target remains open: the next key iteration must improve chroma/temporal representation and confidence calibration, not treat a profile swap or lower refusal threshold as closure.
