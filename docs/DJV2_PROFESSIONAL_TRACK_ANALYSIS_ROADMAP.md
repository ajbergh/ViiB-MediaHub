...
Tempo now has a second representation-level candidate. The native Go multi-feature method combines the existing amplitude-onset envelope and a streaming spectral-flux envelope, scores periodicity independently, and uses 16-second section consensus. It improved strict BPM from 44.09% to 52.76% on tuning and to **56.14% (32/57)** held out. This remains far below the 85% target, and its 5.26% half/double rate misses the ≤5% limit by one track; it is retained only for continued Phase 0 development, not selected as a production default.
...
| 5 | **In progress — replace the tempo estimator and calibrate key.** The expanded tuning corpus proves threshold-only calibration cannot make tempo competitive. Develop against `-analyze-split tuning`, preserve each candidate in a new immutable result file, then evaluate the selected configuration once on held-out data with `-determinism-results <mac.json>,<linux.json>` and `-gate-out <report.json>`. | 1–4 |
