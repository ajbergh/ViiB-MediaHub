...
**Status:** Phases 0–4b landed and merged (through [#43](https://github.com/ajbergh/ViiB-MediaHub/pull/43)); Phase 1 complete; **Phase 5 is open by explicit project-owner override while the Phase 0 corpus/codec acceptance gate remains open.** Tempo/corpus optimization is deferred, not waived.
...
|---|---|---|---|---|---|
| 2026-09-18 | working tree | Tempo candidates, corpus identity audit, native platform evidence | Implemented; release gate remains open | Analysis/benchmark/DJ/API tests; focused vet; Linux tempo/benchmark tests; Windows/Linux generated-fixture comparison; tuning and frozen r5 held-out runs | Clustered voting reaches 104/127 strict on tuning; multi-cycle refinement reaches 109/127 (85.83%) with monotonic confidence, but frozen r5 held-out is 43/54 (79.63%) and 3/54 half/double (5.56%), so neither candidate is promoted. Conservative artist/title matching adds six entries. Recording-group audit finds 14 repeated groups, four crossing splits; r5 preserves original assignments and retains 176 representatives (122 tuning/54 held out), requiring at least 24 additional independent recordings including 13 held-out. Native Windows/Linux agree on all 33 comparable generated fixtures. Added three-platform CI fixture generation and standalone determinism comparison; macOS evidence remains pending. Detailed artifacts and next actions are in `DJV2_TRACK_ANALYSIS_CONCISE_CONTEXT.md`. |
| 2026-09-17 | working tree | Tempo confidence and benchmark evidence integrity | Implemented | Analysis, benchmark, DJ, and API Go tests | Tempo v3 fixes tied-alternative nondeterminism, rounded duplicate alternatives, and inflated confidence caused by replacing the actual runner-up. Benchmark files now retain options and alternate BPM; comparison/gate output preserves options. Determinism validates non-empty results and compares configuration, all recorded scalar diagnostics, and status. Missing/failed analysis cannot earn expected-unknown credit; observed refusals are explicitly exported as `unknown`. Primary tempo selection and thresholds are unchanged; this is reliability work, not a new professional-accuracy claim. |
...
| 2026-09-09 | `djv2/energy-structure-ui` | Phase 7 client, deck UI, recommendations | Merged as [#49](https://github.com/ajbergh/ViiB-MediaHub/pull/49) and [#50](https://github.com/ajbergh/ViiB-MediaHub/pull/50) | `npm run check`; focused Go feature/API/DJ tests | The deck shows persisted energy, supports opt-in cue acceptance, and exposes a local recommendation score with energy, loudness, and phrase-preparation rationale. |
...
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 20 | Complete — local CSV corpus import and first gate record | `go test -count=1 ./internal/analysis/... ./internal/analysisbench ./cmd/analysisbench`; `go vet ./internal/analysis/... ./internal/analysisbench ./cmd/analysisbench`; `analysisbench -import-spotify-corpus <root> ...`; `analysisbench -gate-manifest ... -candidate-results ... -browser-results ... -gate-out ...` | Added a conservative, non-overwriting importer for user-supplied per-folder CSV BPM/key labels and local `.mp3`/`.ogg` media, including Camelot-to-canonical-key conversion, deterministic splits, and unmatched/ambiguous diagnostics. The first 82-track local evidence set produces a recorded failing gate (not Phase 0 completion): throughput and half/double pass; accuracy, browser improvement, corpus coverage, calibration, expected-unknown, and cross-platform determinism do not. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 21 | Complete — periodicity tempo candidate | `go test -count=1 ./internal/analysis/tempo ./internal/analysis/track ./cmd/analysisbench`; tuning/held-out `analysisbench -analyze ... -tempo-method onset-autocorrelation` | Added an explicit, benchmark-only onset-autocorrelation candidate beside the existing peak-interval method. Under the same tuning-only refusal calibration, strict BPM rose from 13.39% to 44.09% on 127 tuning tracks and generalized to 38.60% on 57 held-out tracks. This is a measured improvement, not a selected production default or Phase 0 exit: it remains far below the 85% strict-BPM tripwire. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 22 | Complete — alternate key-profile calibration | `go test -count=1 ./internal/analysis/key ./internal/analysis/track ./cmd/analysisbench`; tuning/held-out `analysisbench -analyze ... -key-profile temperley` | Added an explicit benchmark-only Temperley tonal-profile option beside the existing Krumhansl profile. With the calibrated periodicity tempo settings and key flatness 0.95, Camelot-compatible key accuracy improved from 64.57% to 66.93% on tuning and from 56.14% to 64.91% (37/57) held out; exact held-out key was 40.35%. A six-harmonic chroma-summation experiment improved tuning to 68.50% but regressed held-out compatibility to 59.65%, so it was rejected and is not retained in code. Neither profile is a production default or Phase 0 exit evidence. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 23 | Complete — independently implemented high-resolution HPCP key candidate | `go test -count=1 ./internal/analysis/key ./internal/analysis/track ./cmd/analysisbench`; tuning/held-out `analysisbench -analyze ... -key-extraction hpcp-peaks` | Replaced the direct bin-to-semitone candidate representation with a native Go, streaming implementation informed by Essentia’s published [KeyExtractor](https://essentia.upf.edu/reference/std_KeyExtractor.html) / [HPCP](https://essentia.upf.edu/reference/streaming_HPCP.html) architecture: interpolated local spectral peaks, 36-bin cosine-weighted HPCP, per-frame normalization, and aggregate detuning correction. No Essentia code or runtime dependency is used. With `maxFrequency=3500`, flatness 0.99, and tuning-selected Krumhansl profile, tuning key accuracy was 70.08% exact / 85.83% Camelot-compatible; independent held-out accuracy was **71.93% exact / 85.96% Camelot-compatible (49/57)**, meeting both Phase 0 key tripwires. The raw/direct chroma production path remains unchanged until all remaining Phase 0 exit evidence is complete. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 24 | Complete — multi-feature tempo consensus candidate | `go test -count=1 ./internal/analysis/tempo ./internal/analysis/track ./cmd/analysisbench`; tuning/held-out `analysisbench -analyze ... -tempo-method multifeature-consensus` | Added a native Go, streaming tempo candidate that fuses the amplitude-onset envelope with an independent spectral-flux envelope, scores periodicity in each, and lets 16-second sections vote. With the existing tuning-selected crest 15.00 refusal setting, strict BPM rose from 44.09% to 52.76% on tuning and independently to 56.14% (32/57) held out. The result remains a benchmark-only candidate: it is below the 85% strict-BPM tripwire and its 5.26% held-out half/double rate narrowly misses the ≤5% limit. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 25 | Complete — fractional periodicity peak refinement | `go test -count=1 ./internal/analysis/tempo ./internal/analysis/track ./cmd/analysisbench`; tuning/held-out `analysisbench -analyze ... -tempo-method multifeature-consensus` | Tuning diagnostics showed that 39 of 54 strict-BPM misses were only 1–1.5 BPM away, largely from discrete 125 Hz periodicity lags. The candidate now retains local periodicity maxima, uses bounded quadratic lag interpolation, and votes on a 0.1 BPM grid; a synthetic off-grid-tempo regression test protects that resolution. Strict BPM improved to 70.87% (90/127) on tuning and generalized to **70.18% (40/57)** held out. Held-out half/double improved to 3.51% (2/57), but strict BPM remains below the 85% Phase 0 tripwire. It remains benchmark-only, not a production default. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 26 | Complete — refreshed browser comparator and held-out gate | Local browser export over all 184 manifest tracks; `analysisbench -gate-manifest ... -candidate-results phase0-go-heldout-calibration-n-r2.json -browser-results phase0-browser-results-r2.json -gate-out phase0-gate-report-n-r2.json` | Replaced the obsolete 82-track browser comparator with a local export over the matched 184-track manifest, then generated a new immutable gate record. On held-out data, browser improvements pass (BPM +64.91 points; exact key +29.82 points); half/double (3.51%), key exact (71.93%), Camelot compatibility (85.96%), and 54.42× one-worker throughput pass. The decision remains **do not open Phase 5**: full-corpus strict BPM is 70.18% rather than ≥85%; corpus size/coverage, confidence calibration, expected-unknown, stable-electronic, and macOS/Linux determinism are still unproven. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 27 | Complete — selected half-BPM output-grid candidate | `go test -count=1 ./internal/analysis/tempo ./internal/analysis/track ./cmd/analysisbench`; tuning/held-out `analysisbench -analyze ... -tempo-method multifeature-half-bpm`; `analysisbench -gate-manifest ... -candidate-results phase0-go-heldout-calibration-o-r2.json -browser-results phase0-browser-results-r2.json -gate-out phase0-gate-report-o-r2.json` | The continuous periodicity estimator now has an explicit benchmark-only half-BPM reporting variant, selected because integer-reference labels otherwise left 15 tuning results 0.6–0.8 BPM outside strict tolerance. It reaches 81.10% (103/127) strict BPM on tuning and **77.19% (44/57)** held out; held-out half/double remains 3.51%, browser BPM improvement is 71.93 points, and one-worker throughput is 57.40×. A 60–180 BPM tuning-only range experiment was rejected (45.67% strict, 41.73% half/double) and is not retained. The selected candidate still fails the ≥85% strict-BPM gate and remains non-default. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 28 | Complete — rejected third-onset-cue experiments | `go test -count=1 ./internal/analysis/tempo`; tuning-only `analysisbench -analyze ... -tempo-method multifeature-hfc-half-bpm`; frozen tuning/held-out `analysisbench -analyze ... -tempo-method multifeature-complex-half-bpm` | High-frequency spectral-flux evidence regressed tuning strict BPM to 79.53% (101/127), so it did not receive a held-out pass. Complex spectral difference improved tuning to 84.25% (107/127) with 1.57% half/double, but its frozen held-out result was unchanged at **77.19% (44/57)** with 3.51% half/double while requiring materially more computation. Neither cue is retained. The selected two-cue half-BPM candidate remains Slice 27; Phase 5 remains closed because strict held-out BPM is below 85%. |
| 2026-09-08 | `analysis/phase0-mp3-ogg-evidence-gate` | Phase 0, Slice 29 | Complete — established-pipeline review and committee trial | Research review of Essentia’s BeatTrackerMultiFeature, librosa’s onset/DP tracker, aubio’s streaming tracker, and GPL libKeyFinder; focused Go test plus tuning-only `analysisbench -analyze ... -tempo-method multifeature-committee-half-bpm` | Essentia’s transferable architecture keeps five onset functions independent through beat tracking, then selects the hypothesis with maximum agreement; it is not a weighted feature blend. A clean-room Go candidate adapted that pattern to amplitude, magnitude-flux, and 40-band mel-flux tempo hypotheses. It passed synthetic checks but reached only 73.23% (93/127) strict BPM on tuning, so it was removed and did not consume a held-out pass. Essentia and libKeyFinder are AGPL/GPL respectively and were not copied or linked. The existing Go HPCP/profile key path remains the compatible solution. |
...
| 2026-09-07 | `analysis/foundation-pcm-dsp-persistence` | Phase 1, Slice 3 | Complete | `go test ./internal/db -run '^TestTrackAnalysis'` | Added versioned opaque-artifact and manual-override repositories. Beatgrid-scale payloads remain blobs rather than row-per-beat data; independent BPM/key/beatgrid locks preserve user corrections across re-analysis. PCM/DSP/service work remains. |
| 2026-09-07 | `analysis/foundation-pcm-dsp-persistence` | Phase 1, Slice 4 | Complete | `go test ./internal/db -run '^(TestTrackAnalysis\|TestResolveEffectiveBPM)'` | Added an effective-BPM resolver that returns both provenance and Sync eligibility. Locked manual values outrank completed measured/imported results; legacy integer AI BPM remains a non-Sync fallback, and unknown stays explicit. PCM/DSP/service work remains. |
| 2026-09-07 | `analysis/foundation-pcm-dsp-persistence` | Phase 1, Slice 5 | Complete | `go test ./internal/analysis ./internal/db -run '^(TestDecoderRegistry\|TestTrackAnalysis\|TestResolveEffectiveBPM)'` | Introduced the backend-owned normalized float32 PCM stream contract and explicit extension-keyed decoder registry. It rejects unsupported/duplicate codec registrations and gives tempo/key/waveform stages one shared streaming boundary; concrete decoders, resampling, and source resolution remain. |
| 2026-09-07 | `analysis/foundation-pcm-dsp-persistence` | Phase 1, Slice 6 | Complete | `go test ./internal/analysis` | Added the first production decoder: a bounded RIFF/WAVE PCM16 stream normalizing interleaved samples to float32, registered for `.wav`/`.wave` only. Tests cover sample geometry, chunked EOF behavior, and the honest default unsupported-codec boundary. Resampling, MP3/Vorbis/FLAC, and source resolution remain. |
| 2026-09-07 | `analysis/foundation-pcm-dsp-persistence` | Phase 1, Slice 7 | Complete | `go test ./internal/analysis` | Added deterministic PCM primitives for stereo-aware/all-channel downmix and explicit linear mono resampling, with malformed-geometry and endpoint regression coverage. Streaming composition, source resolution, and additional decoder support remain. |
| 2026-09-07 | `analysis/foundation-pcm-dsp-persistence` | Phase 1, Slice 8 | Complete | `go test ./internal/analysis` | Added canonical local-song source resolution from `song_id`, including safe regular-file checks, a fresh stream opener, and a catalog-hash/size/mtime fingerprint. Plex remains explicitly deferred to an authenticated source adapter; streaming composition and additional decoders remain. |
...
| 2026-09-07 | `analysis/tempo-v1` | Phase 2, Slice 1 | Complete | `go test ./internal/analysis/tempo` | Promoted the Phase 0 onset-interval baseline into a versioned backend tempo-candidate package. It returns fractional BPM or explicit unknown, preserves a future alternate-candidate field, and passes fractional synthetic/silence tests. Opened dependent draft PR [#39](https://github.com/ajbergh/ViiB-MediaHub/pull/39). Corpus accuracy, confidence calibration, and job integration remain. |
| 2026-09-07 | `analysis/tempo-v1` | Phase 2, Slice 2 | Complete | `go test ./internal/analysis/tempo` | Refactored tempo extraction into a chunk-fed onset accumulator that retains only overlap audio and a low-rate onset envelope. Regression coverage proves chunk-boundary output matches one-shot analysis. Corpus accuracy, calibration, and job integration remain. |
| 2026-09-07 | `analysis/tempo-v1` | Phase 2, Slice 3 | Complete | `go test ./internal/analysis/tempo` | Added canonical local-song tempo estimation over the shared resolve/decode/downmix stream. The result returns its source fingerprint for durable persistence; an integration test covers catalog WAV through fractional tempo output. Corpus accuracy, calibration, and job integration remain. |
| 2026-09-07 | `analysis/tempo-v1` | Phase 2, Slice 4 | Complete | `go test ./internal/analysis/tempo` | Connected local tempo estimation to durable `track_analysis` persistence. Measured fractional BPM and confidence are recorded without mutating legacy `songs.bpm`, unanalyzable audio records partial status instead of a fabricated 120 fallback, and source fingerprints track cache freshness. Alternate candidate, configurable range priors, and job integration remain. |
...
| Backend waveform decodes MP3 only; OGG/FLAC/WAV/AAC return a client-side fallback error | [dj_waveform.go:188-206](../backend/internal/api/dj_waveform.go#L188-L206) | Confirmed at snapshot; **superseded 2026-09-07** — waveform generation now runs on the shared `analysis` decoder registry, so MP3, WAV, and Ogg/Vorbis all decode server-side and only formats with no backend decoder defer to the client (§2.4) |
...
> **Resolved 2026-09-07.** This is now done, and it closed the last outstanding Phase 1 backend item. `generateWaveform` no longer carries its own codec switch or its own MP3 decode loop; it calls `analysis.GenerateWaveformOverview` over the shared `DecoderRegistry`, so waveform support *is* the backend codec capability matrix rather than a second list that can drift from it. Consequences worth recording:
>
> - **WAV and Ogg/Vorbis gained server-side waveforms** for free, because Phase 1 had already registered their decoders. `.opus` stays deferred, and the registry keys it separately from `.ogg`, so the conflation fixed in Phase 0 Slice 5 is now structurally impossible to reintroduce rather than merely corrected.
> - **`unsupported codec` is now decided before the file is opened** (`DecoderRegistry.Supports`). Previously a missing file and an unsupported format could surface as each other, which are different lifecycle outcomes (§9.6).
> - **A latent time-axis bug was removed.** The old loop appended one peak per `decoder.Read` call rather than per fixed sample window. `go-mp3` may return a short read without EOF, so peak spacing depended on codec framing instead of on the declared resolution. `PeakAccumulator` carries window state across chunk boundaries; a regression test pins that peaks are identical for chunk sizes from 1 to whole-buffer.
> - **Duration now comes from decoded frames**, not from `decoder.Length()`, so a truncated or mis-tagged file reports what was actually read.
> - The response contract (`duration`/`sampleRate`/`resolution`/`peaks`) and the 422 client-fallback behavior are unchanged, so no frontend change was required.
>
> **One deliberate behavior change to be aware of.** A decodable file containing zero audio frames now returns an error instead of HTTP 200 with an empty `peaks` array. The old path would also write that empty result into `dj_waveform_cache`, so a zero-length file was permanently cached as a valid-but-blank waveform. Failing is both more honest and avoids poisoning the cache, but it surfaces as a 500 rather than the 422 client-fallback code; if that edge case turns out to matter in practice, it should get its own status rather than being folded back into a fake success.
...
### 2.4 Post-merge correctness review (2026-09-07)

The merged Phase 1–4b stack was re-read against this document. Four defects were found and fixed on `analysis/wav-waveform-bridge`. Three of them are the same failure in different places: a component reported a confident answer where the honest answer was "unknown", or discarded a good answer because a status check was too strict.

| # | Defect | Evidence | Fix |
|---|---|---|---|
| 1 | **Key detection never refused atonal material.** `Estimate()` set `Known: true` for any input with energy, gated only on silence. White noise reported "A minor" at confidence **0.377 — higher than every correctly identified synthetic triad (0.074–0.306)**, which inverts the meaning of confidence. A bare click track reported a key at confidence 0.000 with `Known: true`. | Measured on the committed synthetic fixtures: chroma flatness is 0.0003–0.0040 for triads, 0.513 for a click/triad mix, 0.848 for bare clicks, 0.979 for white noise. | Added a chroma-flatness tonality gate (`maxTonalChromaFlatness = 0.70`) in `analysis/key`. Broadband material now returns explicit unknown; all 24 triads still detect, worst-case tonal flatness stays below half the bound. `Flatness` is retained on refusal as a calibration diagnostic. **Threshold is provisional and must be recalibrated on the Phase 0 corpus.** |
| 2 | **`ResolveEffectiveBPM` discarded measured tempo from `partial` records.** It required `status == complete`, but Phase 4b's `combinedStatus` correctly records `partial` when one dimension is unmeasured. A percussion track — measured tempo, no determinable key — therefore fell through to the AI-estimated `songs.bpm`. §9.6 explicitly defines `partial` as a legitimate per-dimension outcome. Latent until Phase 5, which is the first consumer. | `track.combinedStatus` vs. the resolver's status check; no test covered `partial`. | Resolver now accepts `complete` and `partial`, and excludes `pending`/`running` (a claimed row carries the new fingerprint with the previous pass's scalars). End-to-end regression test added. |
| 3 | **The fifth provenance tier was still unnamed**, exactly as §2.3-E warned. `EffectiveBPM` had four sources and no `tempo-descriptor` tier, while `getSongBPM` continued to fall back to `TempoToBPM(song.Tempo)` on its own, bypassing the resolver entirely. `TempoToBPM` returns `DefaultBPMMedium` for any unrecognized descriptor, so it never yields "unknown" — it always fabricates a number. | [scoring.go:326-331](../backend/internal/dj/scoring.go#L326-L331), [constants.go:154](../backend/internal/dj/constants.go#L154) | Added `EffectiveBPMTempoDescriptor`, ranked below `legacy-ai`, never Sync-eligible, plus an `Inferred()` predicate. The resolver now takes an `EffectiveBPMInputs` struct rather than three same-typed pointers, so two inferred tiers cannot be swapped at a call site. **Migrating `getSongBPM` onto the resolver remains Phase 5 scope.** |
| 4 | **Two superseded persist paths remained exported.** `tempo.AnalyzeAndPersistLocalSong` and `key.AnalyzeAndPersistLocalSong` each read-modify-wrote the other dimension's columns and overwrote `algorithm_version` with only their own analyzer's identity — the precise defect `analysis/track` was created to fix, as its package comment documents. Only their own tests called them, so they were a dormant trap rather than an active bug. | `analysis/track/track.go` package doc; caller search found tests only. | Removed both. Their unique coverage (legacy `songs.bpm` separation, alternate metrical candidate, stability, tempo kind, notation derivation) was relocated to `analysis/track` tests. |

**A test-quality problem found alongside these.** The shared `tonalClickTrack` fixture mixed a sustained additive triad at 0.6 amplitude. An additive triad amplitude-modulates at its interval difference frequencies, and at that level the beating dominated the onset envelope: both a 126 and a 128 BPM click track measured **113.038 BPM — the same wrong answer regardless of the real tempo**. The primary both-dimensions happy-path test asserted only `BPM != nil`, so a 13 BPM error passed as green. The fixture level is now 0.2 (key still detects correctly at that level, verified across amplitudes) and the tests assert measured values rather than presence. Two mitigating notes: tempo `stability` collapsed to 0.11–0.13 and confidence to 0.12–0.17 on the bad fixture, so the analyzer did correctly mark the result untrustworthy; and a sustained triad with no clicks is already correctly refused by the existing crest-factor gate.

**Not fixed, recorded as Phase 0/3 calibration work.** Major-key triads score markedly lower confidence than minor-key triads (0.074/0.081 vs 0.305/0.306) on identical synthetic material. That is a profile/correlation asymmetry, not a threshold, and tuning it blind on synthetic fixtures is exactly what §14 forbids. It belongs to open question #5 and the confidence-calibration tripwire.
...
|---|---|---|---|---|
| MP3 | `go-mp3` registered in the shared analysis registry; drives tempo, key, and waveform | `github.com/hajimehoshi/go-mp3` | Apache-2.0; **Pure Go** | **Phase 1 landed** |
| OGG/Vorbis | `jfreymuth/oggvorbis` registered in the shared registry; **server-side waveform as of 2026-09-07** | Existing dependency | MIT; **Pure Go** | **Phase 1 landed** |
| WAV PCM | In-house decoder landed for PCM16 and IEEE float32; **server-side waveform as of 2026-09-07** | In-house bounded RIFF/WAVE reader | In-house MIT; **Pure Go** | **Phase 1 landed**; other sample widths (24-bit, PCM8, PCM32) remain future work |
...
| Opus in Ogg | Scanner ingests `.opus`; `oggvorbis` is **not** an Opus decoder. The `.opus`/`.ogg` conflation is **fixed** — the shared registry keys extensions individually, so `.opus` resolves to no decoder rather than to the Vorbis one | evaluate a permissively licensed pure-Go Opus decoder, or a wazero-hosted `libopus`/`opus-decoder` WASM build | TBD; WASM path uses existing runtime | Explicit unsupported status until validated. The misrouting half of question #9 is closed; the decoder decision remains open |
...
- **A conservative local CSV importer** — `analysisbench -import-spotify-corpus <root> -import-spotify-out <manifest> -import-spotify-evidence-class lawful-real-audio -import-spotify-license private-local` discovers per-folder CSV labels, matches only `.mp3`/`.ogg` tracks by title, converts Camelot keys, makes a stable tuning/held-out split, and reports unmatched or ambiguous rows rather than guessing.
- **A non-overwriting gate artifact** — `-gate-out <report.json>` preserves the tripwire decision next to local evidence instead of leaving it only in terminal output.

#### First local evidence record — gate remains closed

**September 17 reliability follow-up:** `tempo-v3-multifeature-half-bpm` retains the selected primary BPM estimator but fixes alternative tie-breaking, duplicate rounded alternatives, and inflated confidence from reusing the runner-up accumulator. The full 127-track tuning replay produced exactly the same primary BPMs as candidate o: 103 strict matches (81.10%), two half/double errors, six unknown BPM results, and 57.91× real time. Thirty-five confidence values changed; no reported alternative duplicated its primary. Two source decode failures remain explicit. Tempo confidence buckets are not monotonic (52/59 low, 43/52 medium, 8/10 high); key's high bucket is empty. Configuration and alternate BPM are now preserved in result artifacts, and missing/source-failed outputs cannot earn expected-unknown credit. Local immutable evidence: `sample_media/Test Corpus/phase0-go-tuning-reliability-20260917-v3.json` and `phase0-tuning-reliability-comparison-20260917-v3.json`. Analysis/benchmark/DJ/API tests and focused vet passed. No new held-out accuracy claim follows from this pass. Corpus planning correction: extending 184 tracks to 200 requires increasing held-out coverage from 57 to at least 67, so ten of the next 16 tracks must be reserved; the earlier five-track deficit was for the 184-track size only.

The user-supplied `sample_media/Test Corpus` currently contains 215 in-scope media files and 215 CSV rows. Its per-folder CSV labels are treated as user-declared, authoritative Spotify BPM/key data; the importer records that source and does not independently verify Spotify. Conservative title matching produced 184 unambiguous labeled tracks (127 tuning, 57 held-out), 27 unmatched media files, 31 unmatched CSV rows, and four ambiguous media matches. No local media or result artifact is committed.

The first immutable Windows result sets cover all 82 originally matched tracks. The Go analyzer measured 44.64× real-time across that set; the held-out gate measured 47.13× real-time. Two distinct Ogg/Vorbis sources failed early with `ogg: wrong checksum`; they remain explicit `decode_failed` source-integrity results. The terminal-EOF compatibility rule is not applied to, and must not hide, checksum failures.

The recorded first held-out gate decision is **do not open Phase 5**. Go strict BPM was 0.00% (browser baseline 7.41%), Go exact and Camelot-compatible key were each 0.00% (browser exact key 29.63%), and the Go result did not meet either required browser-improvement target. Half/double error was 0.00% and passed; one-worker throughput passed. The corpus is now 16 tracks short of the 200-track minimum, five held-out tracks short of a third, and missing every required genre/case coverage tag. Confidence calibration, expected-unknown behavior, stable-electronic coverage, and macOS/Linux determinism remain unproven.

The expanded 184-track baseline confirms an analyzer-quality problem, not a codec/throughput problem. On its tuning split, the original scanner reached 3.15% strict BPM with 65.35% unknown and 100% key unknown; held-out strict BPM was 0.00% with 80.70% unknown and 100% key unknown. Tuning diagnostics show real tracks sit below the synthetic-only refusal settings (tempo crest median 22.37 versus a 25.00 minimum; key chroma-flatness median 0.897 versus a 0.70 maximum). A tuning-only candidate at crest 15.00 and flatness 0.95 raised reported-key coverage to 88.19% and exact key accuracy to 46.46%, but strict BPM only to 13.39%.

Replacing only the tempo candidate with onset autocorrelation raised strict BPM to 44.09% on tuning and 38.60% on the untouched 57-track held-out split; under the same key setting, exact key was 38.60% held-out. The candidate is retained for further development but is not a production default: both results are well below the Phase 0 85% strict-BPM and 65% exact-key targets. The next tempo iteration must improve onset representation/candidate scoring, not lower another refusal threshold.

For key, the explicit Temperley profile option improved tuning Camelot compatibility from 64.57% to 66.93%, and independently improved held-out compatibility from 56.14% to 64.91% (37/57); exact held-out key was 40.35%. A six-harmonic chroma-summation candidate reached 68.50% on tuning but only 59.65% held out, so it was rejected rather than retained as a scanner option. That profile work established the need for a better chroma representation rather than further threshold-only calibration.

That representation work is now complete for the current 57-track held-out sample. An independent native Go HPCP candidate follows the publicly documented high-level pipeline of interpolated spectral-peak selection, 36-bin cosine-weighted HPCP accumulation, per-frame normalization, and average detuning correction; it does not incorporate Essentia source or binaries. At a tuning-selected 0.99 flatness ceiling, 3500 Hz peak range, and Krumhansl profile, it reached 70.08% exact / 85.83% Camelot-compatible on tuning and **71.93% exact / 85.96% Camelot-compatible (49/57)** held out, with one unknown. It clears the Phase 0 key tripwires. This does not close Phase 0: strict BPM is now 56.14% held out, the corpus is below the required size/coverage, browser-improvement and confidence evidence remain incomplete, and macOS/Linux determinism is unmeasured. The product default stays on the existing direct-chroma algorithm until the full Phase 0 gate selects a complete configuration.

Tempo now has a second representation-level candidate. The native Go multi-feature method combines the existing amplitude-onset envelope and a streaming spectral-flux envelope, scores periodicity independently, and uses 16-second section consensus. Its first result improved strict BPM from 44.09% to 52.76% on tuning and to 56.14% (32/57) held out, but still missed the half/double limit. Tuning-only error analysis then found 39 of the 54 strict misses were just 1–1.5 BPM away, mainly because 125 Hz integer periodicity lags around 128 BPM are about 2 BPM apart. A bounded quadratic interpolation of local lag maxima, retained through a 0.1 BPM vote grid, raised strict BPM to **70.87% (90/127)** on tuning and independently to **70.18% (40/57)** held out; half/double is now 3.51% (2/57). The off-grid resolution has focused synthetic regression coverage. This is a real gain, but it remains well below the 85% strict-BPM gate and is retained only for continued Phase 0 development, not selected as a production default.

The matched browser comparator and a new held-out gate record now exist for the full 184-track manifest, replacing the obsolete 82-track comparison. Browser-relative BPM improvement is 64.91 percentage points and exact-key improvement is 29.82 points; both pass their tripwires. Key exact (71.93%), key Camelot-compatible (85.96%), half/double (3.51%), and 54.42× one-worker throughput also pass. The gate decision is still **do not open Phase 5**: strict BPM is 70.18% rather than ≥85%, the corpus lacks 16 tracks, five more held-out tracks, and required coverage tags, and confidence calibration, expected-unknown behavior, stable-electronic coverage, and macOS/Linux determinism have not yet been demonstrated.

The next selected tempo configuration keeps the continuous estimator but reports on a half-BPM grid calibrated on tuning only. This changes tuning strict BPM to 81.10% (103/127) and independently raises held-out strict BPM to **77.19% (44/57)**; held-out half/double remains 3.51%, browser BPM improvement rises to 71.93 points, and measured throughput is 57.40×. The candidate still fails the 85% strict-BPM tripwire and is not the production default. A 60–180 BPM range experiment was rejected on tuning because it reduced strict BPM to 45.67% and produced 41.73% half/double errors; no range-control code was retained.
...
| 4 | **In progress — expand and tag the local CSV corpus** — the current import has 184 unambiguous local `.mp3`/`.ogg` pairs (127 tuning/57 held-out). Add at least 16 tracks, reserve at least five more held-out tracks, and explicitly tag the §14.2 genres/cases (including `stable-electronic`) without guessing ambiguous filename matches. | lawful audio access and labeling effort |
| 5 | **In progress — replace the tempo estimator and calibrate key.** The expanded tuning corpus proves threshold-only calibration cannot make tempo competitive. The selected fractional multi-feature candidate clears the half/double sub-gate but only reaches 70.18% strict BPM on held-out data; next work must address the remaining harmonic/beat-selection errors using `-analyze-split tuning`, preserve each candidate in a new immutable result file, then evaluate a newly selected configuration once on held-out data with `-determinism-results <mac.json>,<linux.json>` and `-gate-out <report.json>`. | 1–4 |
...
Items 1–3 are complete. The remaining Phase 0 work is corpus expansion/tagging, measured algorithm improvement, calibration, and cross-platform evidence.
...
| 9 | **Reframed.** Opus: is there a viable pure-Go decoder, or is a wazero-hosted `libopus` build the answer? ~~Also fix the existing `.opus`→ogg misrouting in `dj_waveform.go` regardless of outcome~~ | Phase 0 | Same head-to-head form as #8. **Misrouting resolved 2026-09-07** — extension-keyed registry makes `.opus`/`.ogg` conflation structurally impossible; the decoder choice is still open |
...
| 11 | ~~Should the existing `dj_waveform_cache` table be migrated into `track_analysis_artifacts`, or remain specialized and share only the decoder pipeline?~~ | Phase 1 | **Resolved 2026-09-07: share the decoder, keep the table.** Waveform generation now runs on the shared `DecoderRegistry` and a common `PeakAccumulator`, which is where the duplication actually was. The table stays specialized: it is keyed and invalidated on its own lifecycle (lazily on first request, independent of analysis runs), so folding it into `track_analysis_artifacts` would couple waveform cache eviction to analyzer versioning for no benefit. Revisit only if a waveform artifact needs analyzer-version invalidation |
...
| 7 | `djv2/persistent-analysis-library-ux` | 5 | **Started by explicit project-owner override while Phase 0 remains open.** Slices 1–2 completed the `setDeckAnalysis` split (§2.3-F), server-resolved deck hydration, and durable Camelot-first library features. **In Review on `codex/manual-bpm-correction` (rebased onto `4eb78e0`):** loaded-deck fractional BPM entry, ×2/÷2, four-tap BPM, and reset to measured BPM. Manual BPM is stored against the current source fingerprint; stale manual/measured values are hidden after source replacement, and legacy unbound overrides remain unknown. BPM-only writes are conditional and preserve key/grid override state. Scalar BPM edits do not move or re-align beat-grid timestamps; use the existing grid editor for grid changes. Remaining: manual key controls and migrating AI DJ `getSongBPM` onto the effective-BPM resolver (§2.4 finding 3); retain the Phase 0 gate as deferred optimization work. |
...
> **The detailed, audited task list lives in Phase 0 → "Evidence status and execution plan."** The MP3/Ogg measurement harness, browser exporter, Go producer, calibration reporting, and timing are now present. The first local corpus record deliberately fails the gate, so the remaining work is evidence and algorithm improvement rather than a missing-harness implementation.

1. Expand and tag the lawful private/licensed `.mp3`/`.ogg` corpus described in §14.2, including a held-out third and authoritative BPM/key labels.
2. Improve and measure the Go analyzers against the browser baseline, recording strict/accepted BPM, half/double, unknown, exact key, conservative Camelot-compatible key, and confidence calibration against §14.6 on immutable result artifacts.
3. Gather macOS/Linux determinism evidence and record a passing gate before opening `djv2/persistent-analysis-library-ux`. AAC/M4A, Opus, FLAC, and AIFF scope are deliberately deferred beyond the focused MP3/Ogg slice.

Two items added by the §2.4 review, both of which the corpus work must cover:

4. **Recalibrate the two refusal gates on real audio.** `tempo.minOnsetCrestFactor` and `key.maxTonalChromaFlatness` are both provisional thresholds justified by synthetic-fixture margins only. They are what currently produce `unknown` instead of a fabricated answer, so they govern the "≥ 90% unknown on unclassifiable fixtures" tripwire directly — and a threshold tuned on clicks and sine triads is not evidence about music. Report the unknown rate on *real* material separately from the synthetic rate; a gate that refuses genuine tracks is as much a failure as one that accepts noise.
5. **Investigate the major/minor confidence asymmetry** (§2.4). Major triads score roughly a quarter of the confidence of minor triads on identical synthetic material, which will distort any confidence bucketing before it is calibrated. This is open question #5 territory: compare profile variants on the corpus rather than adjusting a constant.

The scheduler split was the critical-path sequencing decision, and it has now landed. The next critical path is evidence rather than feature code: obtain the lawful corpus, measure baseline/detector/codec outcomes on the held-out split, calibrate confidence and the refusal gates, and record a go/no-go decision before opening Phase 5.

**A note on what the review implies about the gate.** Three of the four defects in §2.4 were cases of claiming more certainty than was measured, and none were caught by the existing test suite because the tests asserted that a value was *present* rather than that it was *right*. Synthetic fixtures did catch all of them once the assertions were sharpened, which is a point in favor of the Phase 0 harness — but it also means the accuracy gate should treat "does the detector refuse when it should" as a first-class measured metric, not as a property assumed to follow from correctness elsewhere.
