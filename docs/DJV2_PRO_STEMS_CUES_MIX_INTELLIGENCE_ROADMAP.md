...
- 2026-09-25 — Started the grid-aware generic DJ Beat Jump slice on `codex/djv2-grid-beat-jump`. It adds a numeric 32-beat jump in compact and full controls, uses valid persisted timestamps with the deck offset, and retains BPM timing when grid data cannot serve the requested jump. It does not claim phrase alignment.
- 2026-09-25 — Opened PR #91 for grid-aware DJ Beat Jump controls. Numeric 32-beat controls are available in compact and full layouts; the helper validates and bounds timestamp jumps, applies `beatGridOffset` once, uses the nearest beat for arbitrary playhead positions (earlier beat wins exact ties), and falls back to BPM for invalid/incomplete grids or targets outside the stored grid. Eight focused tests, TypeScript typecheck and `git diff --check` passed locally.
## 1. Product vision
