...
| DJ library True Peak | In Review | Optional, persisted and virtualized dBTP column from the separate BS.1770 artifact. It validates current source identity and exact artifact metadata, then requires `TruePeakStatus == available` and a finite true-peak value independently of integrated-loudness status. Unknown values sort last. It does not use legacy sample-peak proxy fields or change Energy scoring. |
...
- 2026-09-25 — Started the optional DJ Library True Peak column on `codex/dj-library-true-peak-column` from current `origin/main` after PR #101/#102. The batch API validates current, settled BS.1770 artifacts in bounded chunks and checks true-peak availability independently of LUFS. The optional persisted UI column formats dBTP and sorts unknown values last; proxy peak fields and Energy scoring remain unchanged.
## 1. Product vision
