...
| DJ library Structure Status | In review — PR #95 | Optional, persisted and virtualized availability column. `/analysis` checks a batched artifact metadata view against settled analysis status and source fingerprint, then validates only eligible payloads in bounded batches. Legacy artifacts without source fingerprints, stale or unsupported versions, corrupt payloads and invalid sections remain not ready. This reports artifact availability only, not structure quality. |
...
- 2026-09-25 — Opened PR #95 for the optional DJ Library Structure Status column. Analysis artifacts now carry source fingerprints; legacy rows remain unverified. The batch `/analysis` response reports availability only for valid, current-version payloads whose fingerprint matches a complete, partial or failed analysis. Pending/running/unsupported rows, stale or missing fingerprints, corrupt payloads and malformed sections remain unavailable. Focused API, database migration and analysis persistence suites passed, along with TypeScript typecheck and `git diff --check`.
## 1. Product vision
