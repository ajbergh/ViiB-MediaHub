...
- 2026-09-25 — Opened PR #95 for the optional DJ Library Structure Status column. Analysis artifacts now carry source fingerprints; legacy rows remain unverified. Follow-up freshness work resolves current local file stats or cached Plex revision in batched metadata reads, validates all eligible payloads sequentially in bounded chunks, and gates energy detail plus Mix recommendation inputs on current-source identity. Pending/running/unsupported rows, unavailable sources, stale or missing fingerprints, corrupt payloads and malformed sections remain unverified. Validation is in progress.
## 1. Product vision
