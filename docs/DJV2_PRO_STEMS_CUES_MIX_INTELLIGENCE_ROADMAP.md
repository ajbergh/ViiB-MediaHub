...
| DJ library LUFS | In progress — `codex/dj-library-lufs-column` | Optional, persisted and virtualized integrated loudness column from the separate BS.1770 artifact. Only current, settled, measured, supported `loudness-bs1770` artifacts with valid metadata, available loudness status and finite LUFS are shown; missing, legacy, stale, malformed, silent or short measurements remain empty. Unknown values sort last. This does not use the RMS proxy or change Energy scoring. |
...
- 2026-09-25 — Started the optional DJ Library LUFS column on `codex/dj-library-lufs-column` from main after PR #98 (`72a9de8`). The column reads only current measured ITU-R BS.1770-5 integrated loudness; the separate Energy proxy and score remain unchanged.
