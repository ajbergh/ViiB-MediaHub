...
| Dedicated Stem Library workflow | In Review — PR #104 | Separate selected roots and explicit background registry refresh for existing local tracks. Music scans exclude `.viibstems` package subtrees and reconcile old misindexed rows; recursive Artist/Album discovery is bounded and symlink-safe. Only valid packages matching full-file SHA-256 or the existing geometry-matched PCM32 identity become Ready. Scan roots must exactly match enabled Stem Library settings and cannot overlap Music Folders; stem audio is never ingested as music. |
