...
| DJ library LUFS | In review — PR #99 | Optional, persisted and virtualized integrated loudness column from the separate BS.1770 artifact. Only current, settled, measured, supported `loudness-bs1770` artifacts with valid metadata, available loudness status and finite LUFS are shown; missing, legacy, stale, malformed, silent or short measurements remain empty. Unknown values sort last. This does not use the RMS proxy or change Energy scoring. |
