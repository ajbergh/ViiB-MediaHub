...
| Mix/Mashup planning and interoperability | In progress | Mix Next scores confidence-gated BPM/Camelot/Energy Level evidence with Hold/Lift/Reset/Harmonic intents and supports BPM, energy, stems, Camelot, playlist, exact genre, and completed-recently filters. PR #74 adds guarded occupied-deck restoration. PR #76 auditions mix-out/mix-in cue regions through detached headphone sources and applies confidence-qualified tempo-only matching without changing either live deck. PR #78 adds an explicit, guarded Load Candidate action for a pristine, uncued, fully off-air opposite deck and is merged. PR #84 exposes whether manual downbeat evidence is available while stating phase lock remains unavailable without shared scheduled start and phrase-length metadata. PR #87 adds a low-confidence-weighted intro/outro boundary hint using existing timed structure labels; it does not infer vocal safety or phrase alignment. Browser-coordinated starts are not sample-accurate; phrase/bar phase lock, key shift, vocal evidence and Mashup Mode remain open. |
