...
| Mix/Mashup planning and interoperability | In progress | Mix Next scores confidence-gated BPM/Camelot/Energy Level evidence with Hold/Lift/Reset/Harmonic intents and supports BPM, energy, stems, Camelot, playlist, exact genre, and completed-recently filters. The top-candidate headphone preview uses an empty opposite deck and the exact off-air endpoint (PR #67); PR #68 adds cleanup-time ownership checks for routing and mixer controls. PR #74 stages candidate audio off-deck, swaps sources only after a final ownership check, and retains the original source for guarded restoration with its store/engine/stem snapshot. Sync/beatmatch audition, structure/vocal evidence and Mashup Mode remain open. |
...
- Mix planning: a 10-second candidate-only headphone preview is guarded by an empty/pristine opposite deck, the exact fully off-air crossfader endpoint, a dedicated headphone route, disabled master cue and disabled auto-gain. Cleanup now revalidates route ownership at the destructive action boundary as well as deck generation/control ownership; route changes leave the deck untouched. Atomic restoration of an occupied deck remains open, as do reference/incoming synchronized audition, beatmatch/key-shift preview and acceptance-to-load. Structure/vocal-safe evidence is not available; Surprise and Vocal-safe intents stay disabled.
...
- 2026-09-24 — PR #74 opened after detached-source staging and synchronous guarded source swaps replaced active-deck reloads. The exact original source is retained for guarded restoration; stale source callbacks are ignored. Typecheck, 19 focused source/preview tests and `git diff --check` passed locally; full PR CI is pending.
## 1. Product vision
