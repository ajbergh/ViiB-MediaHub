...
**Execution branch:** `codex/testmix-occupied-restore`<br>
...
| Mix/Mashup planning and interoperability | In progress | Mix Next scores confidence-gated BPM/Camelot/Energy Level evidence with Hold/Lift/Reset/Harmonic intents and supports BPM, energy, stems, Camelot, playlist, exact genre, and completed-recently filters. The top-candidate headphone preview uses the exact off-air endpoint (PR #67); PR #68 adds cleanup-time ownership checks. PR #74 stages candidate audio off-deck, swaps sources after a final ownership check, and retains/restores the original source with its store/engine/stem snapshot. Synchronized audition and temporary beatmatch are next; structure/vocal evidence and Mashup Mode remain open. |
