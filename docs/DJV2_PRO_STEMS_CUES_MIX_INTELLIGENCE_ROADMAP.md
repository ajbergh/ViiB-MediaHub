...
| PR 16 — Mix Next completed-recently filter | In progress — PR #70 | Default-off 1–168 hour filter excludes tracks with completed plays in the selected period; response includes `lastPlayed` evidence. UI states that skips/listening events do not update completed-play history. Backend/client tests and typecheck pass; full CI pending. |
| Mix/Mashup planning and interoperability | In progress | Mix Next scores confidence-gated BPM/Camelot/Energy Level evidence with Hold/Lift/Reset/Harmonic intents and supports BPM, energy, stems, Camelot, playlist, exact genre, and completed-recently filters. The top-candidate headphone preview uses an empty opposite deck and the exact off-air endpoint (PR #67); PR #68 adds cleanup-time ownership checks for routing and mixer controls. Atomic restoration of occupied decks remains open, along with sync/beatmatch audition, structure/vocal evidence and Mashup Mode. |
