...
| Mix/Mashup planning and interoperability | In progress | Mix Next scores confidence-gated BPM/Camelot/Energy Level evidence with Hold/Lift/Reset/Harmonic intents and supports BPM, energy, stems, Camelot, playlist, and exact genre filters. The top-candidate headphone preview uses an empty opposite deck and the exact off-air endpoint (PR #67); PR #68 adds cleanup-time ownership checks for routing and mixer controls. Atomic restoration of occupied decks remains open, along with sync/beatmatch audition, structure/vocal evidence and Mashup Mode. |
