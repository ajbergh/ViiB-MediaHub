...
- 2026-09-25 — Opened [PR #106](https://github.com/ajbergh/ViiB-MediaHub/pull/106) for source-aware manual BPM correction, rebased onto main after PR #104. Scalar BPM controls do not move or re-align beat-grid timestamps. Follow-up hardening stores BPM overrides against the source fingerprint, suppresses stale manual/measured BPM across detail, list, transition, and DJ scoring reads, and performs conditional BPM-only writes. Full API/DB Go tests, frontend typecheck, six focused Vitest tests, and `git diff --check` pass locally.
## 1. Product vision
