...
function makeIdea(score: number): NewDJMixIdea {
  return {
    ...
    vector: {
      outgoingTailEnergy: 0.6, incomingHeadEnergy: 0.8, energyDelta: 0.2, loudnessDeltaLu: 1,
      outgoingMixOutConfidence: 0.8, incomingMixInConfidence: 0.7,
    },
    components: [{ name: 'energy', score: 0.8, weight: 0.4, rationale: 'Energy rises by one' }],
...
describe('DJSavedMixIdeas', () => {
  ...
  it('saves the recommendation evidence snapshot and removes it without deck actions', async () => {
    ...
    expect(captured.vector.energyDelta).toBe(0.2);
    ...
    expect(JSON.parse(window.localStorage.getItem(DJ_MIX_IDEAS_STORAGE_KEY)!).ideas[0].score).toBe(0.84);
