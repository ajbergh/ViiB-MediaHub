...
func (d *DB) RequestJobCancellation(id string) (bool, error) {
	...
	// immediately. This also closes the race where the runner would otherwise
	// reject a newly canceling job and leave it stuck forever.
	result, err := d.conn.Exec(`
		UPDATE operation_jobs SET status = ?, message = 'Canceled before start',
		completed_at = ?, updated_at = ? WHERE id = ? AND status = ?
	`, JobStatusCanceled, now, now, id, JobStatusQueued)
	if err != nil {
		return false, err
	}
	rows, err := result.RowsAffected()
	...
	result, err = d.conn.Exec(`
		...
		WHERE id = ? AND status = ?
	`, JobStatusCanceling, now, id, JobStatusRunning)
	if err != nil {
		return false, err
	}
	rows, err = result.RowsAffected()
...
func scanJob(row jobScanner) (Job, error) {
	...
	if parameters.Valid {
		job.Parameters = json.RawMessage(parameters.String)
	}
	if result.Valid {
