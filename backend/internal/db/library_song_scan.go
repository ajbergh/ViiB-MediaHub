...
func scanLibrarySong(row libraryRowScanner) (Song, error) {
	...
	var playCount, lastPlayed, skipCount, legacyBPM, instrumental, moodAnalyzedAt sql.NullInt64
	...
	err := row.Scan(
		...
		&mood, &energy, &tempo, &legacyBPM, &instrumental, &moodAnalyzedAt, &liked,
	...
	if albumArtist.Valid {
		song.AlbumArtist = albumArtist.String
	}
	if trackNum.Valid {
		song.TrackNumber = int(trackNum.Int64)
	}
	if discNum.Valid {
		song.DiscNumber = int(discNum.Int64)
	}
	if genreJSON.Valid && genreJSON.String != "" {
		_ = json.Unmarshal([]byte(genreJSON.String), &song.Genre)
	}
	if year.Valid {
		song.Year = int(year.Int64)
	}
	if originalYear.Valid {
		song.OriginalYear = int(originalYear.Int64)
	}
	song.YearUncertain = yearUncertain.Valid && yearUncertain.Int64 == 1
	if yearAnalyzedAt.Valid {
		song.YearAnalyzedAt = yearAnalyzedAt.Int64
	}
	if replayGainDB.Valid {
		song.ReplayGainDB = replayGainDB.Float64
	}
	if replayPeak.Valid {
		song.ReplayPeak = replayPeak.Float64
	}
	if coverPath.Valid {
		song.CoverPath = coverPath.String
	}
	if playCount.Valid {
		song.PlayCount = int(playCount.Int64)
	}
	if lastPlayed.Valid {
		song.LastPlayed = lastPlayed.Int64
	}
	if skipCount.Valid {
		song.SkipCount = int(skipCount.Int64)
	}
	if fileHash.Valid {
		song.FileHash = fileHash.String
	}
	if mood.Valid {
		song.Mood = mood.String
	}
	if energy.Valid {
		song.Energy = energy.String
	}
	if tempo.Valid {
		song.Tempo = tempo.String
	}
	// songs.bpm is retained for database compatibility only. It contains legacy
	// AI estimates and must never be surfaced as track tempo.
	song.Instrumental = instrumental.Valid && instrumental.Int64 == 1
	if moodAnalyzedAt.Valid {
		song.MoodAnalyzedAt = moodAnalyzedAt.Int64
	}
	song.Liked = liked.Valid && liked.Int64 == 1
	if likedAt.Valid {
		song.LikedAt = likedAt.Int64
	}
	if lastFMListeners.Valid {
		song.LastFMListeners = int(lastFMListeners.Int64)
	}
	if lastFMPlaycount.Valid {
		song.LastFMPlaycount = int(lastFMPlaycount.Int64)
	}
	if lastFMTags.Valid {
		song.LastFMTags = lastFMTags.String
	}
	if lastFMURL.Valid {
		song.LastFMURL = lastFMURL.String
	}
	if lastFMMBID.Valid {
		song.LastFMMBID = lastFMMBID.String
	}
	if lastFMEnrichedAt.Valid {
		song.LastFMEnrichedAt = lastFMEnrichedAt.Int64
	}
	return song, nil
}
