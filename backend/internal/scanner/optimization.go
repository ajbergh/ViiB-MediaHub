...
// metricsCollector provides thread-safe collection of scan performance metrics.
// It supports named timers for timing individual operations and aggregates
// statistics about files and directories processed.
...
// newMetricsCollector creates a new metrics collector ready to track
// performance data for a scan operation.
...
// Start begins a named timer. Call Stop() with the same name to get the
// elapsed duration. Multiple timers can be active simultaneously.
func (m *metricsCollector) Start(name string) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	m.timers[name] = time.Now()
}

// Stop ends a named timer and returns the elapsed duration since Start()
// was called with the same name. Returns 0 if the timer was not started.
func (m *metricsCollector) Stop(name string) time.Duration {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
// AddDirectory records a directory as either scanned (walked) or skipped
// (signature matched). Used for analyzing scan efficiency.
func (m *metricsCollector) AddDirectory(scanned bool) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	if scanned {
		m.metrics.DirectoriesScanned++
	} else {
		m.metrics.DirectoriesSkipped++
	}
}

// AddFile records a file as either processed (metadata extracted) or skipped
// (mtime/size unchanged). Tracks bytes processed for throughput analysis.
func (m *metricsCollector) AddFile(processed bool, size int64) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	if processed {
		m.metrics.FilesProcessed++
		m.metrics.BytesProcessed += size
	} else {
		m.metrics.FilesSkipped++
	}
}

// GetMetrics returns a copy of the collected metrics.
// Safe to call while collection is ongoing.
func (m *metricsCollector) GetMetrics() ScanMetrics {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	return m.metrics
}
