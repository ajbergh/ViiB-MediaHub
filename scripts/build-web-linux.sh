#!/usr/bin/env bash
# Build script for ViiB MediaHub (Web browser build - Linux)
#
# Builds the web-embedded binary that serves a local HTTP server and opens
# the default browser. Does NOT require Wails or WebKitGTK.
...
#   ./scripts/build-web-linux.sh --arch arm64
#   ./scripts/build-web-linux.sh --arch amd64 --debug
#   ./scripts/build-web-linux.sh --clean
...
echo ""
echo "🐧 Building ViiB MediaHub (web browser mode) for Linux ($ARCH)..."
echo ""
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✅ BUILD COMPLETE (Linux web browser build)"
echo "═══════════════════════════════════════════════════════════"
...
if [[ -f "$OUTPUT_BIN" ]]; then
    ...
    echo ""
    echo "  To install:"
    echo "    sudo cp $OUTPUT_BIN /usr/local/bin/viib-mediahub"
else
    echo ""
